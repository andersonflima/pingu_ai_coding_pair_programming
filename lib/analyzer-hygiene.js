...
const { isJavaScriptLikeExtension, isPythonLikeExtension, isGoExtension } = require('./language-profiles');
...
function checkTabs(lines, file) {
  // Go (gofmt) e Makefiles usam tabs por convencao/exigencia; sinaliza-los seria
  // falso positivo. Nessas linguagens o tab nao e problema de higiene.
  const ext = path.extname(String(file || '')).toLowerCase();
  const base = path.basename(String(file || '')).toLowerCase();
  if (isGoExtension(ext) || base === 'makefile' || ext === '.mk') {
    return [];
  }
  const issues = [];
  lines.forEach((line, idx) => {
