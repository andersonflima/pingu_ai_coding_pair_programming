...
function checkBraceScopedUndefinedVariables(lines, file, ext) {
  const issues = [];
  let state = null;
  const moduleScopeVars = collectBraceModuleScopeVariables(lines, ext);
  ...
  for (let idx = 0; idx < lines.length; idx += 1) {
    ...
    if (!state) {
      ...
      state = {
        ...
        allVars: new Set([
          ...moduleScopeVars,
          ...collectBraceScopeAllVariables(lines, idx, declaration.params, ext),
        ]),
        warned: new Set(),
        ext,
      };
...
// Coleta os nomes declarados no escopo de modulo (profundidade 0): const/let/var
// (incluindo require e desestruturacao), nomes de funcao/classe e imports. Esses
// nomes sao visiveis dentro de qualquer funcao do arquivo, entao precisam entrar
// no conjunto de candidatos para nao serem acusados como indefinidos.
function collectBraceModuleScopeVariables(lines, ext) {
  const result = new Set();
  let depth = 0;
  ...
  for (let idx = 0; idx < lines.length; idx += 1) {
    ...
    if (line && depth === 0) {
      ...
      if (declaration && declaration.name) {
        result.add(sanitizeIdentifier(declaration.name));
      }
      const classMatch = line.match(/\bclass\s+([A-Za-z_$][\w$]*)/);
      if (classMatch) {
        result.add(sanitizeIdentifier(classMatch[1]));
      }
    }

    depth += countCurlyBlockDelta(rawLine);
    if (depth < 0) {
      depth = 0;
    }
  }
...
function collectBraceScopeAllVariables(lines, startIdx, params, ext) {
  ...
  for (let idx = startIdx; idx < lines.length; idx += 1) {
    ...
    if (line) {
      ...
      // Parametros e nome de funcoes aninhadas declaradas no corpo: ficam
      // visiveis dentro delas e nao podem ser acusados como indefinidos.
      const nestedDeclaration = parseBraceScopedFunctionDeclaration(rawLine, ext);
      if (nestedDeclaration) {
        (nestedDeclaration.params || []).forEach((param) => {
          const normalized = sanitizeIdentifier(param);
          if (normalized) {
            result.add(normalized);
          }
        });
        if (nestedDeclaration.name) {
          result.add(sanitizeIdentifier(nestedDeclaration.name));
        }
      }
    }
...
function extractScopedAssignmentVars(line, ext) {
  ...
  if (isJavaScriptLikeExtension(lowerExt)) {
    [...source.matchAll(/\bcatch\s*\(\s*([A-Za-z_$][\w$]*)/g)].forEach((match) => names.add(match[1]));
    ...
    // Lista de parametros de arrow (inclusive desestruturada: ([a, b]) => / ({a}) =>).
    [...source.matchAll(/\(([^)]*)\)\s*=>/g)].forEach((match) => {
      extractBoundPatternVars(match[1]).forEach((name) => names.add(name));
    });
  ...
  if (isRustExtension(lowerExt)) {
    ...
    [...source.matchAll(/\b([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(?![=<>:])/g)].forEach((match) => names.add(match[1]));
    // Parametros de closure: |item|, |x, y|, move |x: T| — ligados dentro dela.
    [...source.matchAll(/\|([^|]+)\|/g)].forEach((match) => {
      match[1].split(',').forEach((part) => {
        const param = part.trim().match(/^(?:mut\s+|ref\s+)?([A-Za-z_][A-Za-z0-9_]*)/);
        if (param) {
          names.add(param[1]);
        }
      });
