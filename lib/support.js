...
function countBlockDelta(line) {
  const opens = countMatches(/\bdo\b(?!\s*:)/g, line);
...
function looksCommentWorthyAssignment(rhs) {
  ...
  // So vale um comentario de manutencao quando o passo e genuinamente nao-obvio.
  // Atribuicoes simples (uma unica chamada, um acesso a propriedade, um operador
  // ou um literal de colecao) sao auto-explicativas e gerariam apenas ruido.
  // Expressoes curtas sao auto-explicativas mesmo quando compostas (guardas como
  // `Array.isArray(x) ? x : []` ou coercoes como `String(x).trim()`); so passos
  // substanciais justificam um comentario de intencao.
  if (normalized.length < 45) {
    return false;
  }
  const callCount = (normalized.match(/\b\w+\s*\(/g) || []).length;
  const hasTernary = /\?[^?]*:/.test(normalized);
  const logicalCount = (normalized.match(/&&|\|\||\?\?|\|>/g) || []).length;
  const hasNestedCall = /\b\w+\s*\([^()]*\b\w+\s*\(/.test(normalized);
...
function describeFunctionDocArgument(arg) {
  const rawName = String(arg || '').trim();
  if (!rawName) {
    return 'Valor de entrada utilizado no fluxo desta funcao.';
  }
  ...
  if (/^(opts?|options?|config|cfg)$/.test(normalizedLower)) {
    return 'Configuracao recebida para ajustar o comportamento desta funcao.';
  }
  if (/^(payload|data|dados)$/.test(normalizedLower)) {
    return 'Carga de dados recebida e validada antes de seguir no fluxo.';
  }
  if (/^(ctx|context|contexto|state|runtime_?state)$/.test(normalizedLower)) {
    return 'Estado compartilhado usado para coordenar esta etapa.';
  }
  if (/^(id|user_?id|usuario_?id|client_?id|cliente_?id|room_?id|sala_?id)$/.test(normalizedLower)) {
    return 'Identificador usado para localizar o recurso tratado nesta etapa.';
  }

  const readable = humanizeIdentifier(normalizedName).toLowerCase();
  return `Valor de entrada para ${readable} dentro desta funcao.`;
}
...
function snippetFunctionDoc(name, params, context = {}) {
  ...
  const isElixirDoc = isElixirExtension(context.ext || '') || Boolean(context.specSignature);
  const argLines = hasArgs
    ? args.map((arg) => (
      isElixirDoc
        ? `  - \`${arg}\`: ${describeFunctionDocArgument(arg)}`
        : `  - ${arg}: ${describeFunctionDocArgument(arg)}`
    ))
    : ['  - Nenhum argumento recebido.'];
  const summary = context.summary || `${functionDescriptionFromName(functionName)}.`;
  const returnDescription = context.returnDescription || `Retorna o resultado produzido por ${humanizeIdentifier(functionName)} conforme o contrato da funcao.`;
  const specSignature = String(context.specSignature || '').trim();

  if (isElixirDoc) {
    const contractLines = specSignature
      ? [
        '',
        '  ## Contrato',
        `  \`${specSignature}\``,
...
function snippetFunctionComment(name, params, ext = '') {
  ...
  const functionLabel = humanizeIdentifier(name || 'funcao').toLowerCase();
  return [
    `  ${prefix} Funcao ${name}: ${functionDescriptionFromName(name)}.`,
    `  ${prefix} Argumentos: ${paramsText}.`,
    `  ${prefix} Contrato: manter o retorno consistente com o fluxo de ${functionLabel}.`,
  ].join('\n');
}
function snippetFunctionSpec(name, params, ext, context = {}) {
  ...
  const paramsText = params.length > 0
    ? params.map((_, index) => paramTypes[index] || 'term()').join(', ')
    : '';
  const returnType = context.returnType || 'term()';
...
function functionDescriptionFromName(name) {
  ...
  return `Executa a etapa principal de ${subject} preservando o contrato esperado`.trim();
}
...
// Palavra-chave de controle de fluxo em linguagens C-like, usada para distinguir
// um cabecalho de funcao de um bloco de controle na analise de escopo.
