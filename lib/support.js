...
function isDependencyDeclarationLine(line, ext = '') {
  ...
  const lowerExt = String(ext || '').toLowerCase();
  const isJavaScriptLike = ['.js', '.jsx', '.mjs', '.cjs', '.ts', '.tsx'].includes(lowerExt);
  if (!trimmed) {
    return false;
  }
  if (/^(?:import\b|from\b.+\s+import\b|use\b|package\b|require\b|module\.exports\b)/.test(trimmed)) {
    return true;
  }
  ...
  return /^(?:const|let|var)\s+(?:\{[^}]+\}|[A-Za-z_$][A-Za-z0-9_$]*)\s*=\s*require\s*\(/.test(trimmed)
