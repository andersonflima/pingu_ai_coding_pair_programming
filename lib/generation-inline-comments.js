...
function classifyCStyle(trimmed) {
  ...
  return classifyByPatterns(clean, [
    ['return', /^return\b\s*(.*)$/],
    ['def', /^(?:export\s+)?(?:default\s+)?(?:pub\s+)?(?:async\s+)?(?:func|fn|function)\s*\*?\s*([A-Za-z_$][\w$]*)/],
    ['def', /^(?:export\s+)?class\s+([A-Za-z_$][\w$]*)/],
    ['def', /^(?:export\s+)?(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:async\s*)?(?:function|\([^)]*\)\s*=>)/],
    ['assign', /^(?:export\s+)?(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=/],
    ['assign', /^([A-Za-z_$][\w$.\[\]]*)\s*:=/],
    ['call', C_CALL],
    ['assign', C_ASSIGN],
  ]);
...
function braceFamily({ commentPrefix, headerRegexes, isDocLine, classify, describe }) {
  return {
    commentPrefix,
    docPlacement: 'before',
    bodyIndentUnit: 2,
