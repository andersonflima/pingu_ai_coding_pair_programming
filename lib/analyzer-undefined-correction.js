...
const { suggestSimilarIdentifier, levenshteinDistance } = require('./identifier-similarity');
...
function resolveUndefinedVariableSuggestion(lines, lineNumber, unknown, candidates) {
  ...
  const suggestion = suggestSimilarIdentifier(unknown, candidates);
  return isGenuineTypoSuggestion(unknown, suggestion) ? suggestion : null;
}

// Aceita a sugestao apenas quando ela e plausivelmente um erro de digitacao do
// nome pretendido: distancia de edicao pequena e proporcional ao tamanho, e
// comprimento parecido. Sem isso, casamentos frouxos (p.ex. subsequencia) fazem
// o detector "corrigir" `error` para um nome longo e nao relacionado.
function isGenuineTypoSuggestion(unknown, suggestion) {
  const source = String(unknown || '');
  const target = String(suggestion || '');
  if (!source || !target || source === target) {
    return false;
  }
  // Identificadores muito curtos (contadores, escapes de regex, parametros de
  // uma letra) quase nunca sao erro de digitacao real e geram ruido.
  if (source.length < 3) {
    return false;
  }
  const maxDistance = source.length <= 4 ? 1 : source.length <= 8 ? 2 : 3;
  if (Math.abs(source.length - target.length) > maxDistance) {
    return false;
  }
  return levenshteinDistance(source.toLowerCase(), target.toLowerCase()) <= maxDistance;
}
