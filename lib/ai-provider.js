...
const { createCodexAiProvider } = require('./ai-provider-codex');
const { createClaudeAiProvider } = require('./ai-provider-claude');
...
function createAiProvider(deps = {}) {
  ...
  const codexProvider = deps.codexProvider || createCodexAiProvider(deps.codexDeps || {});
  const claudeProvider = deps.claudeProvider || createClaudeAiProvider(deps.claudeDeps || {});
  ...
  function readProviderMode(env = process.env) {
    const mode = normalizeEnvValue(env.PINGU_AI_PROVIDER || 'codex');
    if (['openai', 'copilot', 'codex', 'claude', 'anthropic', 'auto'].includes(mode)) {
      return mode;
    }
    return 'codex';
  }
  ...
  function resolvePreferredProvider(env = process.env) {
    ...
    if (mode === 'codex') {
      return providerAvailable(codexProvider, env) ? codexProvider : null;
    }
    if (mode === 'claude' || mode === 'anthropic') {
      return providerAvailable(claudeProvider, env) ? claudeProvider : null;
    }

    if (providerAvailable(codexProvider, env)) {
      return codexProvider;
    }
    if (providerAvailable(claudeProvider, env)) {
