...
function createCopilotAiProvider(deps = {}) {
  ...
  function readProviderCommand(env = process.env) {
    // Precedencia: env > provider.command no .pingurc.json > default.
    return normalizeEnvValue(resolveProviderCommand(undefined, env)) || DEFAULT_PROVIDER_COMMAND;
  }
  ...
  function readProviderModel(env = process.env) {
    // Precedencia: env > provider.model no .pingurc.json.
    return normalizeEnvValue(resolveProviderModel(undefined, env));
  }
  ...
  function readProviderKind(command, env = process.env) {
    // Precedencia: env > provider.kind no .pingurc.json > inferencia pelo binario.
    const kind = normalizeEnvValue(resolveProviderKind(undefined, env)).toLowerCase();
