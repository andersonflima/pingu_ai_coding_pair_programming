...
function createCommentTaskTools(deps) {
  ...
  function normalizePinguDirectiveIntent(rawIntent) {
    const intent = String(rawIntent || '').trim().toLowerCase().replace(/_/g, '-');
    if (['comment', 'comments', 'comente', 'comenta', 'comentar', 'doc', 'docs', 'document', 'documenta', 'documente', 'documentar'].includes(intent)) {
      return { marker: ':', intent: 'comment' };
    }
    if (['code', 'fix', 'refactor', 'refatora', 'corrige', 'corrigir'].includes(intent)) {
      return { marker: ':', intent: 'code' };
    }
    if (['context', 'ctx', 'blueprint', 'scaffold'].includes(intent)) {
      return { marker: '**', intent: 'context' };
    }
    if (['terminal', 'term', 'shell', 'cmd', 'command', 'run'].includes(intent)) {
      return { marker: '*', intent: 'terminal' };
    }
    if (['test', 'tests', 'unit-test', 'unit-tests'].includes(intent)) {
      return { marker: ':', intent: 'test' };
    }
    return null;
  }

  function normalizePinguDirectiveInstruction(intent, instruction) {
    const text = String(instruction || '').trim();
    if (intent === 'test') {
      return `gerar testes unitarios para ${text}`;
    }
    if (intent === 'comment') {
      return /\bcomment|coment|document|documenta|\bdoc\b/i.test(text) ? text : `comment ${text}`;
    }
    return text;
  }

  function stripPinguDirectiveCommentSyntax(line, ext) {
    const lowerExt = analysisExtension(ext);
    ...
    const openBlockMatch = text.match(/^\/\*\s*([\s\S]*)$/);
    if (openBlockMatch) {
      return String(openBlockMatch[1] || '').trim();
    }
    const htmlMatch = text.match(/^<!--\s*([\s\S]*?)\s*-->\s*$/);
    if (htmlMatch) {
      return String(htmlMatch[1] || '').trim();
    }
    if (isMermaidExtension(lowerExt)) {
      const mermaidMatch = text.match(/^%%\s*(.+)$/);
      return mermaidMatch ? String(mermaidMatch[1] || '').trim() : '';
    }
    if (supportsHashComments(lowerExt) || ['.tf'].includes(lowerExt)) {
      const hashMatch = text.match(/^#\s*(.+)$/);
      return hashMatch ? String(hashMatch[1] || '').trim() : '';
    }
    if (supportsSlashComments(lowerExt)) {
      const slashMatch = text.match(/^\/\/\s*(.+)$/);
      return slashMatch ? String(slashMatch[1] || '').trim() : '';
    }
    if (lowerExt === '.lua') {
      const luaMatch = text.match(/^--\s*(.+)$/);
      return luaMatch ? String(luaMatch[1] || '').trim() : '';
    }
    if (lowerExt === '.vim') {
      const vimMatch = text.match(/^"\s*(.+)$/);
      return vimMatch ? String(vimMatch[1] || '').trim() : '';
    }
    const fallbackMatch = text.match(/^(?:#|\/\/|--|")\s*(.+)$/);
    return fallbackMatch ? String(fallbackMatch[1] || '').trim() : '';
  }

  function matchPinguDirective(line, ext) {
    const body = stripPinguDirectiveCommentSyntax(line, ext);
    if (!body || /^\\s\s*@?pingu\b/i.test(body)) {
      return null;
    }

    const match = body.match(/^@?pingu(?::\s*|\s+)([A-Za-z][A-Za-z_-]*)\s+(.+)$/i);
    if (!match) {
      return null;
    }

    const intent = normalizePinguDirectiveIntent(match[1]);
    if (!intent) {
      return null;
    }

    const rawMarker = `@pingu ${String(match[1] || '').trim().toLowerCase()}`;
    return {
      rawMarker,
      marker: intent.marker,
      instruction: normalizePinguDirectiveInstruction(intent.intent, match[2]),
    };
  }

  function matchCommentTask(line, ext) {
    // Entrada: linha e extensao | Saida: marcador normalizado + instrucao, ou null.
    const pinguDirective = matchPinguDirective(line, ext);
    if (pinguDirective) {
      return pinguDirective;
    }
  ...
  function buildTerminalTask(lines, file, lineNumber, instruction, ext, strictAiCommentAction, rawMarker = '*') {
    // Entrada: contexto do comentario e arquivo | Saida: issue de terminal ou null.
    ...
    if (aiGeneratedTask.aiFailure) {
      ...
      return buildAiRequiredIssue(
        ...
        lineNumber,
        'terminal_task',
        aiGeneratedTask.aiFailureMessage || `Sem retorno de acao de terminal no modo offline para ${analysisExtension(file)}.`,
      );
    }
    ...
    if (requiresAiTerminalTask) {
      return buildAiRequiredIssue(
        ...
        lineNumber,
        'terminal_task',
        `Sem retorno de action.run_command no modo offline para ${analysisExtension(file)}.`,
      );
    }
  ...
  function checkCommentTask(lines, file, opts = {}) {
    // Entrada: linhas + caminho + opcoes | Saida: lista de issues candidatas.
    ...
    lines.forEach((line, idx) => {
      ...
      if (generatedTask.aiFailure) {
        issues.push(buildAiRequiredIssue(
          ...
          lineNumber,
          'comment_task',
        generatedTask.aiFailureMessage || `Sem retorno de implementacao valida para ${analysisExtension(file)} no modo offline.`,
        ));
        return;
      }
      ...
      if (!strictAiCommentAction && !generatedTask.inlineComment && commentTaskAlreadyApplied(lines, idx, generatedTask, ext)) {
        return;
      }
  ...
  function buildAiRequiredIssue(file, line, feature, message = '') {
    // Entrada: localizacao + feature | Saida: issue padronizada de acao offline indisponivel.
    return {
      file,
      line,
      severity: 'error',
      kind: 'ai_required',
      message: message || `Fluxo offline para ${feature} em ${analysisExtension(file)} ainda sem cobertura ativa.`,
      suggestion: 'Habilite cobertura local equivalente no mapa offline da linguagem ou execute o fluxo manualmente.',
      snippet: '',
