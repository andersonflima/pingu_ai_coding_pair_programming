...
function checkRedundantConstructs(lines, file, kind, opts = {}) {
  ...
  source.forEach((rawLine, index) => {
    ...
    if (isJavaScriptLikeExtension(kind)) {
      ...
      if (hasParseIntWithoutRadix(code)) {
        issues.push(buildIssue(file, lineNumber, 'parseint_no_radix',
          'parseInt sem o argumento de base (radix)',
          'Informe a base explicitamente, p.ex. parseInt(valor, 10), para evitar interpretacao ambigua.'));
      }
    }
  });
  return issues;
...
function hasParseIntWithoutRadix(code) {
  const text = maskStrings(code);
  const pattern = /\bparseInt\s*\(/g;
  let match;
  while ((match = pattern.exec(text)) !== null) {
    // Ignora acesso a membro/propriedade (ex.: obj.parseInt).
    const before = text[match.index - 1];
    if (before === '.') {
      continue;
    }
    const args = readBalancedArgs(text, pattern.lastIndex - 1);
    if (args === null) {
      continue;
    }
...
function readBalancedArgs(text, openParenIndex) {
  let depth = 0;
  let inString = '';
  for (let cursor = openParenIndex; cursor < text.length; cursor += 1) {
    const char = text[cursor];
    if (inString) {
      if (char === '\\') {
        cursor += 1;
    ...
    if (char === '"' || char === '\'' || char === '`') {
      ...
      depth -= 1;
      if (depth === 0 && char === ')') {
        return text.slice(openParenIndex + 1, cursor);
      }
    }
  }
  return null;
...
function hasTopLevelComma(args) {
  return splitTopLevelParams(args).length >= 2;
}
