...
function removeCommentsFromSource(source, request = {}) {
  ...
  const supportsPythonDocstrings = supportsPythonDocstringComments(ext);
  ...
  return String(source || '')
    ...
    .split('\n')
    .map((line, index) => {
      if (supportsPythonDocstrings) {
        const withoutDocstring = stripPythonDocstringComment(line, { inBlock: inPythonDocstring });
        inPythonDocstring = withoutDocstring.inBlock;
        if (withoutDocstring.removed) {
          return {
            original: line,
            text: '',
            removed: true,
    ...
    .filter((line) => !line.removed && (line.text.trim() !== '' || String(line.original || '').trim() === ''))
    .map((line) => line.text)
    .join('\n');
}
...
function supportsPythonDocstringComments(ext) {
  return ext === '.py';
}

function stripPythonDocstringComment(line, state) {
  ...
  if (state && state.inBlock) {
    const end = text.indexOf(state.inBlock);
    return {
      text: '',
      inBlock: end >= 0 ? false : state.inBlock,
      removed: true,
    };
  }
  ...
  const afterOpening = trimmed.slice(quote.length);
  const closesOnSameLine = afterOpening.includes(quote);
  return {
    text: '',
    inBlock: closesOnSameLine ? false : quote,
    removed: true,
  };
}
