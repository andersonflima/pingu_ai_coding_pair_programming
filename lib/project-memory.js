...
function loadContextMemory(projectRoot) {
  const pinguContextFile = path.join(projectRoot, '.pingu', 'context.md');
  if (pathExists(pinguContextFile)) {
    const document = safeReadFile(pinguContextFile);
    return {
      filePath: pinguContextFile,
