...
const { supportsCommentTaskIntent } = require('./language-capabilities');
...
const { buildInlineCommentedFunction, isCommentInstructionForFollowingCode } = require('./generation-inline-comments');
...
function synthesizeFromCommentTask(instruction, ext, lines = [], sourceFile = '', options = {}) {
  ...
  if (!preferAiCommentTask && lineIndex >= 0 && isCommentInstructionForFollowingCode(instruction)) {
    const inlineCommented = buildInlineCommentedFunction({
      lines,
      triggerIndex: lineIndex,
      ext: normalizedExt,
      file: sourceFile,
      buildDocstring: (header, docContext) => (header
        ? buildLeadingFunctionDocumentation(header.name, header.params, instruction, normalizedExt, docContext || {})
        : ''),
    });
    if (inlineCommented && inlineCommented.snippet) {
      return {
        snippet: inlineCommented.snippet,
        action: inlineCommented.action,
        inlineComment: true,
        disableMaintenanceComments: true,
        semanticIntent,
        intentIR,
        generationValidation: { ok: true, reasons: [] },
      };
    }
  }
...
function buildRenderedFunctionSnippet(name, params, instruction, ext, body, renderOptions = {}) {
  ...
  const renderedBody = ext.toLowerCase() === '.php'
    ? phpVariableizeBody(body, params)
    : body;
  ...
  functionLines.push(`${bodyIndent}${renderedBody}`);
  if (closer === 'none') {
...
function phpVariableizeBody(body, params) {
  // No corpo gerado, as referencias aos parametros usam nomes nus (a, b);
  // em PHP elas precisam do prefixo $. Substitui apenas os nomes dos parametros,
  // preservando os ja prefixados.
  return (Array.isArray(params) ? params : []).reduce((acc, param) => {
    const name = sanitizeIdentifier(param);
    if (!name) {
      return acc;
    }
    return acc.replace(new RegExp(`(^|[^$\\w])${escapeRegExp(name)}\\b`, 'g'), `$1$$${name}`);
  }, String(body || ''));
}
...
function functionSignature(name, params, instruction, ext, options = {}) {
  ...
  if (lowerExt === '.java') {
    const javaParams = params.map((param) => `Object ${sanitizeIdentifier(param)}`).join(', ');
    return [`public Object ${sanitizeIdentifier(name)}(${javaParams}) {`, '}'];
  }
  if (lowerExt === '.cs') {
    const csParams = params.map((param) => `object ${sanitizeIdentifier(param)}`).join(', ');
    return [`public object ${sanitizeIdentifier(name)}(${csParams}) {`, '}'];
  }
  if (['.kt', '.kts'].includes(lowerExt)) {
    const ktParams = params.map((param) => `${sanitizeIdentifier(param)}: Any`).join(', ');
    return [`fun ${sanitizeIdentifier(name)}(${ktParams}): Any {`, '}'];
  }
  if (lowerExt === '.swift') {
    const swiftParams = params.map((param) => `_ ${sanitizeIdentifier(param)}: Any`).join(', ');
    return [`func ${sanitizeIdentifier(name)}(${swiftParams}) -> Any {`, '}'];
  }
  if (['.scala', '.sc'].includes(lowerExt)) {
    const scalaParams = params.map((param) => `${sanitizeIdentifier(param)}: Any`).join(', ');
    return [`def ${sanitizeIdentifier(name)}(${scalaParams}): Any = {`, '}'];
  }
  if (lowerExt === '.php') {
    const phpParams = params.map((param) => `$${sanitizeIdentifier(param)}`).join(', ');
    return [`function ${sanitizeIdentifier(name)}(${phpParams}) {`, '}'];
  }
  return [`function ${sanitizeIdentifier(name)}(${paramsText}) {`, '}'];
}
...
function baseHint(expr, ext) {
  ...
  if (['.c', '.cpp', '.h', '.hpp', '.java', '.cs', '.php'].includes(ext.toLowerCase())) {
    return `return ${expr};`;
  }
  if (['.py', '.js', '.jsx', '.ts', '.tsx', '.go', '.kts', '.kt', '.lua', '.vim', '.swift', '.scala', '.sc'].includes(ext.toLowerCase())) {
    return `return ${expr}`;
  }
  return expr;
}
...
const DECLARATION_AUTHORITY_ISSUE_KINDS = new Set([
  'function_doc',
  'function_spec',
  'unit_test_signature',
]);
...
const DEFAULT_AUTOMATIC_AI_SYNTAX_BUNDLE_MIN_ISSUES = 1;
const DEBUG_AUTOMATIC_AI = /^(?:1|true|yes|on)$/i.test(String(process.env.PINGU_DEBUG_AUTOMATIC_AI || ''));

function isAiModeOff(env = process.env) {
  const value = String((env && env.PINGU_AI_MODE) || '').trim().toLowerCase();
  return ['0', 'false', 'off', 'disable', 'disabled', 'no', 'nao', 'não'].includes(value);
}
...
function readAutomaticAiSyntaxBundleMinIssues(env = process.env) {
  const parsed = Number.parseInt(
    String(env.PINGU_AUTOMATIC_AI_SYNTAX_BUNDLE_MIN_ISSUES || DEFAULT_AUTOMATIC_AI_SYNTAX_BUNDLE_MIN_ISSUES),
    10,
  );
  if (!Number.isFinite(parsed)) {
    return DEFAULT_AUTOMATIC_AI_SYNTAX_BUNDLE_MIN_ISSUES;
  }
  return Math.max(0, parsed);
}

function isSyntaxLikeIssue(issue) {
  const kind = String(issue && issue.kind || '').trim();
  const message = String(issue && issue.message || '').trim();
  return kind.startsWith('syntax_') || /\bsyntax error\b/i.test(message);
}

function buildAutomaticSyntaxBundleIssue(file, syntaxIssues = [], options = {}) {
  const items = Array.isArray(syntaxIssues) ? syntaxIssues : [];
  const minIssues = Math.max(0, Number.parseInt(String(options.minIssues || DEFAULT_AUTOMATIC_AI_SYNTAX_BUNDLE_MIN_ISSUES), 10) || 0);
  if (minIssues <= 0 || items.length < minIssues) {
    return null;
  }

  const diagnostics = items
    .map((issue) => {
      const line = Number(issue && issue.line || 1);
      const message = String(issue && issue.message || '').trim();
      if (!message) {
        return '';
      }
  ...
  const firstLine = items.reduce((minLine, issue) => {
    const line = Number(issue && issue.line || minLine || 1);
    if (!Number.isFinite(line) || line <= 0) {
      return minLine;
    }
    return Math.min(minLine, line);
  }, Number.MAX_SAFE_INTEGER);

  return {
    file,
    line: Number.isFinite(firstLine) && firstLine > 0 ? firstLine : 1,
    severity: 'error',
    kind: 'syntax_bundle_repair',
    message: `Multiplos erros de sintaxe detectados (${items.length}); aplicar reparo consolidado no arquivo.`,
    suggestion: 'Corrija os erros de sintaxe de forma consistente no bloco afetado preservando semantica.',
    snippet: '',
    action: {
      op: 'write_file',
      target_file: file,
    ...
    metadata: {
      ...
      syntaxIssueCount: items.length,
    },
  };
}

function buildAutomaticResolutionIssues(mergedIssues, file, options = {}) {
  const list = Array.isArray(mergedIssues) ? mergedIssues : [];
  const shouldUseAiSyntaxBundle = Boolean(options.shouldUseAiSyntaxBundle);
  const syntaxBundleMinIssues = Math.max(0, Number.parseInt(String(options.syntaxBundleMinIssues || DEFAULT_AUTOMATIC_AI_SYNTAX_BUNDLE_MIN_ISSUES), 10) || 0);
  if (!shouldUseAiSyntaxBundle) {
    return list;
  }

  const syntaxIssues = list.filter((issue) => isSyntaxLikeIssue(issue));
  const syntaxBundleIssue = buildAutomaticSyntaxBundleIssue(file, syntaxIssues, {
    minIssues: syntaxBundleMinIssues,
  });
  if (!syntaxBundleIssue) {
    return list;
  }

  const nonSyntaxIssues = list.filter((issue) => !isSyntaxLikeIssue(issue));
  return [syntaxBundleIssue, ...nonSyntaxIssues];
}

function buildAutomaticIssueInstruction(issue) {
  const baseInstruction = buildFollowUpInstruction(issue);
  const kind = String(issue && issue.kind || '').trim();
  const message = String(issue && issue.message || '').trim();
  ...
  const metadata = issue && issue.metadata && typeof issue.metadata === 'object'
    ? issue.metadata
    : {};
  const isSyntaxIssue = isSyntaxLikeIssue(issue);
  ...
  const details = [];
  if (message) {
    details.push(`diagnostico_sintaxe="${message}"`);
  }
  if (suggestion) {
    details.push(`sugestao_sintaxe="${suggestion}"`);
  }
  if (Array.isArray(metadata.syntaxDiagnostics) && metadata.syntaxDiagnostics.length > 0) {
    details.push(`diagnosticos_sintaxe="${metadata.syntaxDiagnostics.join(' || ')}"`);
  }
  if (!details.length) {
    return `${baseInstruction} priorize o diagnostico real de sintaxe informado pelo parser/linter.`;
  }
  return `${baseInstruction} use literalmente o output de sintaxe: ${details.join(' | ')}`;
}

function extractAutomaticSyntaxErrorOutput(issue) {
  const message = String(issue && issue.message || '').trim();
  const suggestion = String(issue && issue.suggestion || '').trim();
  const metadata = issue && issue.metadata && typeof issue.metadata === 'object'
    ? issue.metadata
  ...
  const diagnostics = Array.isArray(metadata.syntaxDiagnostics) ? metadata.syntaxDiagnostics : [];
  return [message, suggestion, ...diagnostics]
...
function resolveAutomaticIssuesWithAi(lines, file, issues = [], options = {}) {
  ...
  const aiModeOff = isAiModeOff(env);
  // Sem chamadas de IA permitidas, nem sondamos a disponibilidade do provider
  // (evita um spawnSync de probe durante a analise passiva).
  const hasAiConfiguration = allowAiCalls && hasOpenAiConfiguration();
  ...
  const automaticAiSyntaxBundleMinIssues = readAutomaticAiSyntaxBundleMinIssues(options.env || process.env);
  ...
  if (aiModeOff) {
    if (DEBUG_AUTOMATIC_AI) {
      process.stderr.write('[PINGU_DEBUG_AI] ai mode off: skipping automatic resolution\n');
    }
    return mergedIssues;
  }
  ...
  const resolutionIssues = buildAutomaticResolutionIssues(mergedIssues, file, {
    shouldUseAiSyntaxBundle: automaticFixPolicy.shouldUseAi && hasAiConfiguration && allowAiCalls,
    syntaxBundleMinIssues: automaticAiSyntaxBundleMinIssues,
  });
  const resolved = resolutionIssues.map((issue, index) => {
    if (DEBUG_AUTOMATIC_AI) {
    ...
    const issueInstruction = buildAutomaticIssueInstruction(issue);
    const aiResolution = resolveAiIssueFix({
      ...
      effectiveInstruction: issueInstruction,
      syntaxErrorOutput: extractAutomaticSyntaxErrorOutput(issue),
    });
...
function normalizeAutomaticAiAction(issue, file, action) {
  ...
  const originalAction = issue && issue.action && typeof issue.action === 'object'
    ? issue.action
    : null;
  const originalOp = String(originalAction && originalAction.op || '').trim();

  // Para sincronizacao de contrato (doc/spec/test signature), a declaracao atual e a fonte da verdade.
  // Mantemos a acao estrutural do analyzer para evitar que um provider remoto escale para write_file
  // e reverta rename manual no simbolo declarado.
  if (DECLARATION_AUTHORITY_ISSUE_KINDS.has(issueKind) && originalOp) {
    return originalAction;
  }

  // Sempre preserve replace_range emitido pelo analyzer local para manter escopo minimo da correcao.
  if (originalOp === 'replace_range') {
    return originalAction;
  }
