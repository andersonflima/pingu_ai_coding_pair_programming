...
const { isAnalyzeAiEnabled } = require('./pingu-config');
const {
  LANGUAGE_PROFILES,
  analysisExtension,
  unitTestStyle,
...
function createUnitTestCoverageChecker(helpers = {}) {
  ...
  function checkUnitTestCoverage(lines, file) {
    ...
    // A analise passiva e local e rapida por default: nao invoca a IA para gerar
    // testes (o que spawna o Copilot a cada analise) — e nem sequer sonda a
    // disponibilidade do provider quando nao vai usa-lo. Habilite com
    // PINGU_ANALYZE_AI (ou "analyzeAi": true no .pingurc.json); o enriquecimento
    // por IA acontece sob demanda no fix.
    const analyzeAiAllowed = isAnalyzeAiEnabled(file, process.env);
    const canUseAiForUnitTests = analyzeAiAllowed && hasOpenAiConfiguration && hasOpenAiConfiguration();
    const aiPolicy = resolveAiFeaturePolicy('unit_test', process.env, {
      hasOpenAiConfiguration: canUseAiForUnitTests,
    });
    const mustUseAiForUnitTests = analyzeAiAllowed
      && (Boolean(requiresAiForFeature && requiresAiForFeature(file, 'unit_test')) || aiPolicy.mustUseAi);
    const shouldUseAiForUnitTests = analyzeAiAllowed && (mustUseAiForUnitTests || aiPolicy.shouldUseAi);
        ...
        return [
          buildUnitTestIssue(
            ...
            aiGeneratedUnitTests.action && aiGeneratedUnitTests.action.target_file
              ...
              : targetFile,
            'Cobertura de testes unitarios ausente',
            'Pingu pode gerar testes cobrindo o contrato publico identificado. Aplique a sugestao se quiser cria-los.',
  ...
  function buildUnitTestSignatureIssue(file, line, targetFile, candidate, currentArity, targetArity, updatedLine, rangeOverride) {
    ...
    const testFileLabel = path.basename(String(targetFile || '')) || 'teste existente';
    return {
      file,
      line,
      severity: 'warning',
      kind: 'unit_test_signature',
      message: `Teste existente ${testFileLabel} referencia ${normalizedCandidate} com assinatura antiga.`,
      suggestion: `Atualizar a chamada em ${testFileLabel} para ${normalizedCandidate}/${targetArity}? Aplique a correcao se quiser que o Pingu ajuste o teste.`,
      snippet: updatedLine,
      metadata: {
        ...declarationContract,
  ...
  function inferPythonBehaviorTest(file, candidates, candidate) {
    const source = readTextFileCached(file);
    if (!source) {
      return null;
    }
