...
function createUnitTestCoverageChecker(helpers = {}) {
  ...
  function checkUnitTestCoverage(lines, file) {
    ...
    // PINGU_ANALYZE_AI; o enriquecimento por IA acontece sob demanda no fix.
    const analyzeAiAllowed = /^(?:1|true|on|yes)$/i.test(String(process.env.PINGU_ANALYZE_AI || '').trim());
