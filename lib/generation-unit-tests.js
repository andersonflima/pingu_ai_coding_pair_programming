...
function createUnitTestCoverageChecker(helpers = {}) {
  ...
  function findUnitTestSignatureDriftIssuesInFile(file, relatedFile, ext, candidates) {
    ...
    for (const candidate of candidates) {
      ...
      if (!Number.isFinite(expectedArity) || expectedArity < 0) {
        continue;
      }
  ...
  function collectUnitTestCallArityOperationsInFile(lines, ext, candidateName, expectedArity, candidate) {
    const normalizedExpectedArity = Number(candidate && candidate.arity);
    if (!candidateName || !Number.isFinite(normalizedExpectedArity) || normalizedExpectedArity < 0) {
      return [];
    }
    ...
    for (const invocation of invocations) {
      if (invocation.arity === normalizedExpectedArity) {
        continue;
      }

      const nextArguments = buildTargetArityArguments(invocation.arguments, normalizedExpectedArity, candidate, lowerExt);
      operations.push({
        currentArity: invocation.arity,
        updatedArity: Math.max(0, Math.floor(normalizedExpectedArity)),
  ...
  function collectUnitTestInvocationsInLines(lines, candidateName) {
    ...
    for (let lineIdx = 0; lineIdx < normalizedLines.length; lineIdx += 1) {
      ...
      while (cursor < currentLine.length) {
        const index = currentLine.indexOf(candidateName, cursor);
        if (index < 0) {
          break;
        }

        const before = index > 0 ? currentLine[index - 1] : '';
        const after = index + candidateName.length < currentLine.length
          ? currentLine[index + candidateName.length]
          : '';
        ...
        let openIndex = index + candidateName.length;
        while (openIndex < currentLine.length && /\s/.test(currentLine.charAt(openIndex))) {
          openIndex += 1;
        }
        if (currentLine.charAt(openIndex) !== '(') {
          cursor = openIndex + 1;
          continue;
        }

        const range = resolveMatchingParenRangeInLines(normalizedLines, lineIdx, openIndex);
        if (!range) {
          cursor = openIndex + 1;
          continue;
        }
