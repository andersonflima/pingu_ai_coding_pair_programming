...
function createUnitTestCoverageChecker(helpers = {}) {
  ...
  function isSignatureAwareUnitTestCandidate(candidate) {
    ...
    if (!candidateName || !Number.isFinite(arity) || arity < 0) {
      return false;
    }
  ...
  function findUnitTestSignatureDriftIssuesInFile(file, relatedFile, ext, candidates) {
    ...
    lines.forEach((line, lineIndex) => {
      let mutableLine = String(line || '');
      for (const candidate of candidates) {
        const candidateUpdate = rewriteUnitTestLineForCandidateSignature(mutableLine, ext, candidate);
        if (!candidateUpdate.changed) {
          continue;
        }

        mutableLine = candidateUpdate.line;
        issues.push(buildUnitTestSignatureIssue(
          file,
          lineIndex + 1,
          relatedFile,
          candidate,
          candidateUpdate.currentArity,
          candidateUpdate.updatedArity,
          candidateUpdate.line,
        ));
      }
    });
  ...
  function rewriteUnitTestLineForCandidateSignature(line, ext, candidate) {
    const expectedArity = Number(candidate && candidate.arity);
    const candidateName = sanitizeIdentifier(candidate && candidate.name || '').trim();
    if (!candidateName || !Number.isFinite(expectedArity) || expectedArity < 0) {
    ...
    if (isElixirExtension(lowerExt)) {
      const exportPattern = new RegExp(`(:${escapeRegExp(candidateName)}\\s*,\\s*)(\\d+)`, 'g');
      let match = null;
      while ((match = exportPattern.exec(line)) !== null) {
        const currentArity = Number(match[2]);
        if (!Number.isFinite(currentArity) || currentArity === expectedArity) {
          continue;
        }
        ...
        operations.push({
          ...
          replacement: String(Math.max(0, Math.floor(expectedArity))),
          currentArity,
          updatedArity: Math.max(0, Math.floor(expectedArity)),
        });
      }
    }

    const callOperations = collectUnitTestCallArityOperations(line, lowerExt, candidateName, expectedArity, candidate);
    operations.push(...callOperations);

    if (operations.length === 0) {
      return { changed: false };
    }

    const updatedLine = applyLineRewriteOperations(line, operations);
    if (updatedLine === String(line || '')) {
      return { changed: false };
    }
    ...
    return {
      changed: true,
      line: updatedLine,
      currentArity: operations[0] && operations[0].currentArity,
      updatedArity: operations[0] && operations[0].updatedArity,
    };
  }
  ...
  function collectUnitTestCallArityOperations(line, ext, candidateName, expectedArity, candidate) {
    ...
    invocations.forEach((invocation) => {
      if (invocation.arity === expectedArity) {
        return;
      }

      const nextArguments = buildTargetArityArguments(invocation.arguments, expectedArity, candidate, ext);
      operations.push({
  ...
  function buildUnitTestSignatureIssue(file, line, targetFile, candidate, currentArity, targetArity, updatedLine) {
  ...
  function collectUnitTestInvocations(line, candidateName) {
    ...
    const safeName = escapeRegExp(candidateName);
    if (!safeName) {
      return [];
    }
    ...
    while (cursor < normalizedLine.length) {
      const index = normalizedLine.indexOf(candidateName, cursor);
      if (index < 0) {
        break;
      }
      const before = index > 0 ? normalizedLine[index - 1] : '';
      const after = index + candidateName.length < normalizedLine.length
        ? normalizedLine[index + candidateName.length]
        : '';

      if (isIdentifierCharacter(before) || isIdentifierCharacter(after)) {
        cursor = index + candidateName.length;
        continue;
      }

      let openIndex = index + candidateName.length;
  ...
  function extractElixirTestCandidates(lines) {
    ...
    lines.forEach((line, index) => {
      ...
      seen.add(name);
      candidates.push({ name, arity: countParams(match[2] || ''), line: index + 1, moduleName, symbolKind: 'function' });
    });
  ...
  function extractGoTestCandidates(lines) {
    ...
    lines.forEach((line, index) => {
      ...
      seen.add(name);
      candidates.push({ name, arity: countParams(match[2]), line: index + 1, symbolKind: 'function' });
    });
  ...
  function extractVimTestCandidates(lines) {
    ...
    lines.forEach((line, index) => {
      ...
      seen.add(rawName);
      candidates.push({ name: rawName, arity: countParams(match[2]), line: index + 1, symbolKind: 'function' });
    });
