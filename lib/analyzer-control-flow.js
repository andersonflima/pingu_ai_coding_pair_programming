...
function checkControlFlowSmells(lines, file, kind, opts = {}) {
  ...
  return [
    ...
    ...checkControlFlowInFinally(source, file, kind, focusRange),
  ];
}
...
// ---------------------------------------------------------------------------
// return/break/continue dentro de finally
// ---------------------------------------------------------------------------

function checkControlFlowInFinally(lines, file, kind, focusRange) {
  if (!isJavaScriptLikeExtension(kind)) {
    return [];
  }
  const issues = [];
  ...
  lines.forEach((rawLine, index) => {
    ...
    // finally { return ... } numa unica linha.
    if (/\bfinally\s*\{[^{}]*\b(?:return|break|continue)\b/.test(trimmed)) {
      pushFinallyIssue(issues, file, index + 1, focusRange);
    } else if (/(?:^|[;)]\s*|\belse\s+)(?:return|break|continue)\b/.test(trimmed) && nearestEnclosingIsFinally(blockStack)) {
      pushFinallyIssue(issues, file, index + 1, focusRange);
    }

    const blockKind = classifyBlockOpener(trimmed);
...
function classifyBlockOpener(trimmed) {
  if (/\bfinally\b/.test(trimmed)) {
    return 'finally';
  }
  if (/\bfunction\b/.test(trimmed) || /=>\s*\{?\s*$/.test(trimmed) || /\)\s*=>/.test(trimmed)) {
    return 'function';
  }
  return 'other';
}

function nearestEnclosingIsFinally(blockStack) {
  for (let cursor = blockStack.length - 1; cursor >= 0; cursor -= 1) {
    if (blockStack[cursor] === 'function') {
      return false;
    }
    if (blockStack[cursor] === 'finally') {
      return true;
    }
  }
  return false;
...
function pushFinallyIssue(issues, file, line, focusRange) {
  if (!isLineInFocus(focusRange, line)) {
    return;
  }
  issues.push({
    file,
    line,
    severity: 'warning',
    kind: 'control_flow_in_finally',
    message: 'return/break/continue dentro de finally',
    suggestion: 'Desviar o fluxo no finally engole excecoes e sobrescreve o retorno do try/catch; mova essa logica para fora do finally.',
    snippet: '',
    action: { op: 'insert_before' },
  });
}
