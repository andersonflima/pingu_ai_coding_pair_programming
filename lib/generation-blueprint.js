  ...
  function resolveBlueprintSourceExtension(projectRoot, file) {
    ...
    if (
      [
        ...
        '.c', '.cpp', '.h', '.hpp',
        '.sh',
        // Linguagens sem scaffold CRUD nativo: preservar a extensao mantem o
        // blueprint document-only (sem fallback enganoso para .js) em vez de
        // espalhar arquivos JavaScript num projeto Java/PHP/Kotlin/etc.
        '.java', '.cs', '.kt', '.kts', '.swift', '.scala', '.sc', '.php',
      ].includes(currentExt)
    ) {
      if (currentExt === '.tsx') {
        return '.ts';
