...
function parseElixirFunctionDocArgumentNames(docLines) {
  ...
  for (let idx = 0; idx < lines.length; idx += 1) {
    ...
    if (/^##\s*Argumentos\b/i.test(trimmed)) {
    ...
    const argMatch = trimmed.match(/^\-\s*([a-z_][a-zA-Z0-9_?!]*)\b/i);
...
function isElixirFunctionDocOutdated(docRange, declaration, lines = []) {
  ...
  const parsedDoc = parseElixirFunctionDocArgumentNames(buildElixirAnnotationRangeLines(contextLines, docRange));
...
function resolveCrossLanguageFunctionDocRange(lines, declarationLineIdx, declaration, ext) {
  ...
  if (supportsSlashComments(lowerExt) || supportsHashComments(lowerExt)) {
...
function isCrossLanguageLineComment(line, ext) {
  ...
  if (supportsSlashComments(lowerExt) && /^\/\/\/?/.test(normalizedLine)) {
    return true;
  }
  if (supportsHashComments(lowerExt) && /^#/.test(normalizedLine)) {
    return true;
  }
  return false;
}
...
function resolveCrossLanguageJavaScriptDocRange(lines, declarationLineIdx) {
  let cursor = declarationLineIdx - 1;
  while (cursor >= 0 && !String(lines[cursor] || '').trim()) {
    cursor -= 1;
  }
  if (cursor < 0) {
  ...
  const initial = String(lines[cursor] || '').trim();
  if (!initial || !(initial.startsWith('/**') || initial.endsWith('*/') || /^\*/.test(initial))) {
    return null;
  }
  ...
  if (initial.startsWith('/**')) {
    ...
    for (let idx = cursor + 1; idx < declarationLineIdx; idx += 1) {
      if (String(lines[idx] || '').includes('*/')) {
        return {
          startLine: cursor,
          endLine: idx,
        };
  ...
  if (initial.includes('*/')) {
    for (let start = cursor - 1; start >= 0; start -= 1) {
      const startLine = String(lines[start] || '');
      const startTrim = startLine.trim();
      if (!startTrim) {
        continue;
      }
      if (startTrim.startsWith('/**')) {
        return {
          startLine: start,
          endLine: cursor,
        };
      }
      if (!(startTrim.startsWith('*') || /^\/\//.test(startTrim) || startTrim.startsWith('*/'))) {
        return null;
      }
    }
    return null;
  ...
  if (/^\*/.test(initial)) {
    for (let start = cursor - 1; start >= 0; start -= 1) {
      const startTrim = String(lines[start] || '').trim();
      if (!startTrim) {
        continue;
      }
      if (startTrim.startsWith('/**')) {
...
function isCrossLanguageFunctionDocOutdated(docRange, declaration, ext, lines = []) {
  ...
  const expectedParams = normalizeDocumentedFunctionParams(declaration, ext);
  const expectedCount = expectedParams.length;

  if (expectedCount === 0) {
    return parsed.hasArgumentsSection && !parsed.hasNoArgsPlaceholder;
  }

  if (parsed.argNames.length !== expectedCount) {
    return true;
  }

  return parsed.argNames.some((argumentName, index) => argumentName !== sanitizeIdentifier(expectedParams[index]));
}
...
function parseGenericFunctionDocSignature(docLines, ext) {
  ...
  for (let idx = 0; idx < linesSource.length; idx += 1) {
    ...
    if (
      inArgumentsSection
      && /^(?:##\s*)?(?:retorno|returns?|resultado|sa[ií]da|output|outputs?|throws?|raises?|exce[cç][aã]o|erros?)\b/i.test(strippedLine)
    ) {
      inArgumentsSection = false;
      continue;
    }

    if (/^nenhum argumento recebido/i.test(strippedLine) || /^sem argumento|^sem argumentos/i.test(strippedLine) || /no arguments/i.test(strippedLine)) {
      hasNoArgsPlaceholder = true;
      continue;
    }

    if (/^[-*]\s*([a-z_][a-zA-Z0-9_?!]*)\s*(?:\([^:]+\))?\s*:\s*/i.test(strippedLine)) {
      const match = strippedLine.match(/^[-*]\s*([a-z_][a-zA-Z0-9_?!]*)\s*(?:\([^:]+\))?\s*:\s*/i);
      if (match && match[1]) {
        hasArgumentsSection = true;
        inArgumentsSection = true;
        argNames.push(sanitizeIdentifier(match[1]));
      }
      continue;
    }

    if (inArgumentsSection) {
      const inlineMatch = strippedLine.match(/^([a-z_][a-zA-Z0-9_?!]*)\s*:\s*/i);
      if (inlineMatch && inlineMatch[1]) {
        argNames.push(sanitizeIdentifier(inlineMatch[1]));
        continue;
      }
    }

    const paramMatch = strippedLine.match(/@param\b(?:\s+\{[^}]+\})?\s+([a-z_][a-zA-Z0-9_?!]*)/i);
    if (paramMatch && paramMatch[1]) {
      hasArgumentsSection = true;
      argNames.push(sanitizeIdentifier(paramMatch[1]));
    }
  }
...
function hasCrossLanguageFunctionDocSignature(parsedSignature, ext) {
  ...
  return Boolean(parsedSignature
    && (parsedSignature.hasArgumentsSection
      ...
      || (Array.isArray(parsedSignature.argNames) && parsedSignature.argNames.length > 0)));
}
...
function parsePythonFunctionDocSignature(docLines) {
  ...
  for (let idx = 0; idx < linesSource.length; idx += 1) {
    ...
    if (inArgumentsSection) {
      ...
      const match = trimmed.match(/^\s*([a-z_][a-zA-Z0-9_?!]*)\s*(?:\([^:]+\))?\s*:\s*/i);
      if (match && match[1]) {
        argNames.push(sanitizeIdentifier(match[1]));
      }
    }
...
function parseJavaScriptFunctionDocSignature(docLines) {
  ...
  const isDocBlock = /\/\*\*/.test(clean) || /\* /.test(clean);
  linesSource.forEach((line) => {
    const trimmed = String(line || '').trim();
    if (!trimmed) {
      return;
    }
    const match = trimmed.match(/@param\b(?:\s+\{[^}]+\})?\s+([a-z_][a-zA-Z0-9_?!]*)\b/i);
    if (match && match[1]) {
      argNames.push(sanitizeIdentifier(match[1]));
    }
  });
  const hasArgumentsSection = argNames.length > 0;
  const hasReturnTag = /@returns?\b/.test(clean);
...
function checkFunctionSpecs(lines, file, opts = {}) {
  ...
  for (let idx = 0; idx < lines.length; idx += 1) {
    ...
    const annotationRange = resolveElixirAnnotationRange(lines, idx, '@spec');
    if (annotationRange) {
