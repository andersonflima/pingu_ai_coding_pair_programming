...
const { checkMissingAwait, checkAwaitInLoop, checkAsyncArrayMethods } = require('./analyzer-async');
const { checkPythonNamingHazards } = require('./analyzer-python-naming');
const { checkHardcodedSecrets } = require('./analyzer-secrets');
const { checkSecurityIssues } = require('./analyzer-security');
const { checkRedundantConstructs } = require('./analyzer-redundant');
const { checkFunctionalReassignment, checkNestedConditionDepth, checkCyclomaticComplexity } = require('./analyzer-complexity');
const { checkUndefinedVariables } = require('./analyzer-undefined-variables');
const {
  resolveDisabledKinds,
  isFormattingHygieneEnabled: isFormattingHygieneEnabledForFile,
  isAnalyzeAiEnabled,
  resolveMaxLineLength,
} = require('./pingu-config');
const { applyInlineSuppressions } = require('./inline-suppressions');
...
function analyzeText(filePath, text, opts = {}) {
  ...
  const maxLineLength = resolveMaxLineLength(analyzedFile, opts.maxLineLength, DEFAULT_MAX_LINE_LENGTH, process.env);
  ...
  if (isStructuredTextKind(analyzedKind)) {
    ...
    if (!activeLanguage) {
      ...
      appendIssues('hardcoded_secrets', () => checkHardcodedSecrets(lines, analyzedFile, analyzedKind, { focusRange }));
      appendIssues('security', () => checkSecurityIssues(lines, analyzedFile, analyzedKind, { focusRange }));
      ...
      appendIssues('async_array_method', () => checkAsyncArrayMethods(lines, analyzedFile, analyzedKind, { focusRange }));
      ...
      if (runLightAnalysisDeepPass) {
        ...
        appendIssues('high_complexity', () => checkCyclomaticComplexity(lines, analyzedFile));
      }
  ...
  // A analise passiva e local e rapida por default: nao invoca a IA (que
  // spawna um processo por issue gerável e travaria o editor a cada tecla).
  // O enriquecimento por IA acontece sob demanda no fluxo de fix/geracao; quem
  // quiser resolucao por IA ja durante a analise habilita com PINGU_ANALYZE_AI.
  const allowAnalyzeAiCalls = isAnalyzeAiEnabled(analyzedFile, process.env);
  const aiResolvedIssues = activeLanguage
    ? resolveAutomaticIssuesWithAi(lines, analyzedFile, issues, {
      allowAiCalls: allowAnalyzeAiCalls,
    })
    : issues;
  if (DEBUG_ANALYZE_STEPS) {
  ...
  const disabledKinds = resolveDisabledKinds(analyzedFile, process.env);
  // Higiene redundante com formatters (prettier/black/gofmt/rustfmt) fica off por
  // default para reduzir ruido; habilite via PINGU_ENABLE_FORMATTING_HYGIENE ou
  // "formattingHygiene": true no .pingurc.json.
  if (!isFormattingHygieneEnabledForFile(analyzedFile, process.env)) {
    FORMATTER_REDUNDANT_KINDS.forEach((kind) => disabledKinds.add(kind));
  }
  const dedup = [];
  const seen = new Set();
  ...
  // Supressao inline por comentario (pingu-disable-line / -next-line / -file),
  // aplicada por ultimo para valer sobre todos os detectores.
  return applyInlineSuppressions(dedup, lines);
}
// Higiene que os formatters ja cobrem melhor e universalmente; off por default
// para nao competir com prettier/black/gofmt/rustfmt nem gerar ruido.
const FORMATTER_REDUNDANT_KINDS = ['trailing_whitespace', 'tabs', 'long_line', 'large_file'];
...
function checkCrossLanguageFunctionDocs(lines, file, opts = {}) {
  ...
  for (let idx = 0; idx < lines.length; idx += 1) {
    ...
    if (!documentedRange) {
      ...
      if (!isPublicFunctionContract(declaration, ext, exportedNames)) {
        idx = declarationEnd;
        continue;
      }
      issues.push({
...
function checkFunctionMaintenanceComments(lines, file, opts = {}) {
  ...
  const issues = [];
  const focusRange = opts.focusRange || null;
  // So o contrato publico recebe sugestao de comentario; helpers internos nao.
  const exportedNames = isJavaScriptLikeExtension(ext)
    ? new Set(collectJavaScriptExportNames(lines.join('\n')))
    : null;
  lines.forEach((line, idx) => {
...
// Verdadeiro quando a funcao faz parte do contrato publico do modulo: exportada
// em JS/TS, nao-privada por convencao em Python (sem prefixo _ exceto dunder),
// publica em Elixir (def, nao defp) ou em Go (identificador com inicial
// maiuscula). Outras linguagens mantem o comportamento anterior (sempre
// elegiveis).
function isPublicFunctionContract(declaration, ext, exportedNames) {
  const name = String(declaration.name || '');
  if (isElixirExtension(ext)) {
    return declaration.visibility !== 'defp';
  }
  if (isPythonLikeExtension(ext)) {
    return !/^_/.test(name) || /^__.+__$/.test(name);
  }
  if (isGoExtension(ext)) {
    return /^[A-Z]/.test(name);
  }
  if (isJavaScriptLikeExtension(ext)) {
    return exportedNames ? exportedNames.has(name) : true;
  }
  return true;
}
