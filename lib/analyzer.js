...
function analyzeText(filePath, text, opts = {}) {
  ...
  const allowAnalyzeAiCalls = /^(?:1|true|on|yes)$/i.test(String(process.env.PINGU_ANALYZE_AI || '').trim());
  ...
  // default para reduzir ruido; habilite com PINGU_ENABLE_FORMATTING_HYGIENE.
  if (!isFormattingHygieneEnabled(process.env)) {
...
function isFormattingHygieneEnabled(env = process.env) {
  return /^(?:1|true|on|yes)$/i.test(String(env.PINGU_ENABLE_FORMATTING_HYGIENE || '').trim());
}
