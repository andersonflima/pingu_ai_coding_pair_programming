...
function resolveCrossLanguageBlockCommentDocRange(lines, declarationLineIdx, ext) {
  ...
  if (endTrimmed.includes('/*') && endTrimmed.includes('*/') && endTrimmed.lastIndexOf('/*') < endTrimmed.lastIndexOf('*/')) {
    return {
      startLine: cursor,
      endLine: cursor,
    };
...
function parseGenericFunctionDocSignature(docLines, ext) {
  ...
  for (let idx = 0; idx < linesSource.length; idx += 1) {
    ...
    const bulletMatch = strippedLine.match(/^[-*+]\s*`?([a-z_][a-zA-Z0-9_?!]*)`?(?:\([^:]+\))?\s*(?:[:\-–—]\s*|\s{2,}|=>\s+)/i);
    if (bulletMatch && bulletMatch[1]) {
      hasArgumentsSection = true;
      inArgumentsSection = true;
      argNames.push(sanitizeIdentifier(bulletMatch[1]));
      continue;
    }
...
function parseJavaScriptFunctionDocSignature(docLines) {
  ...
  const hasArgumentsSection = genericSignature.hasArgumentsSection || argNames.length > 0;
  const hasReturnTag = genericSignature.hasReturnTag || /@returns?\b/.test(clean);

  return {
    argNames: [...new Set([...genericSignature.argNames, ...argNames])],
    hasArgumentsSection,
