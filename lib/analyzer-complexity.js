...
const PY_DECISION_TOKENS = /\b(?:if|elif|for|while|except)\b|\band\b|\bor\b/g;

// Conta pontos de decisao por funcao Python (delimitada por indentacao) e
// sinaliza as que passam do limiar.
function checkPythonComplexity(lines, file) {
  ...
  const flush = (indent) => {
    while (stack.length > 0 && stack[stack.length - 1].indent >= indent) {
      ...
      if (complexity > CYCLOMATIC_THRESHOLD) {
        issues.push(buildComplexityIssue(file, frame.startLine, complexity));
      }
    }
  };
  ...
  for (let index = 0; index < source.length; index += 1) {
    ...
    flush(indent);
    if (stack.length > 0) {
      stack[stack.length - 1].decisions += countMatches(PY_DECISION_TOKENS, removeInlineComment(raw));
    }
    if (/^\s*(?:async\s+)?def\b/.test(raw)) {
      stack.push({ startLine: index + 1, indent, decisions: 0 });
    }
  }
  flush(0);
  return issues;
...
function buildComplexityIssue(file, startLine, complexity) {
  return {
    file,
    line: startLine,
    severity: 'info',
...
function checkCyclomaticComplexity(lines, file) {
  ...
  if (isPythonLikeExtension(ext)) {
    return checkPythonComplexity(lines, file);
  }
