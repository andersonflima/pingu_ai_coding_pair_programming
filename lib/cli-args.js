...
const OPTION_DEFINITIONS = Object.freeze([
  ...
  { names: ['--lsp-ai-fix'], key: 'lspAiFixMode', withValue: false, transform: (_value, args) => {
    if (args) {
      args.output = 'json';
    }
    return true;
  } },
  { names: ['--prompt-task'], key: 'promptTaskMode', withValue: false, transform: (_value, args) => {
    if (args) {
      args.output = 'json';
    }
    return true;
  ...
  { names: ['--lsp'], key: 'lspMode', withValue: false, transform: () => true },
