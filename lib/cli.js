...
function resolveCliCommand(rawArgs) {
  ...
  if (['analyze', 'a', 'fix', 'f', 'prompts', 'prompt', 'comments', 'comment', 'p', 'taxonomy', 'offline', 'profile', 'init', 'context', 'ctx', 'doctor', 'help'].includes(normalized)) {
...
function buildDoctorChecks() {
  ...
  const providerCommand = readProviderCommand(providerMode);
  const providerHint = providerAvailable ? 'operacional' : providerAvailabilityHint(providerMode, providerCommand);
  ...
  return [
    ...
    },
    {
      ...
      ok: ['auto', 'openai', 'copilot', 'codex', 'claude', 'anthropic'].includes(providerMode),
...
function readProviderCommand(providerMode, env = process.env) {
  if (providerMode === 'codex' || providerMode === 'auto') {
    return String(env.PINGU_CODEX_COMMAND || 'codex').trim();
  }
  if (providerMode === 'claude' || providerMode === 'anthropic') {
    return String(env.PINGU_CLAUDE_COMMAND || env.PINGU_ANTHROPIC_COMMAND || 'claude').trim();
  }
  if (providerMode === 'copilot') {
    return String(env.PINGU_COPILOT_COMMAND || 'copilot').trim();
  }
...
function providerAvailabilityHint(providerMode, command) {
  if (providerMode === 'openai') {
    return 'configure OPENAI_API_KEY para API remota';
  }
  if (providerMode === 'codex' || providerMode === 'auto') {
    return `instale/autentique o CLI e valide com ${command} doctor`;
  }
  if (providerMode === 'claude' || providerMode === 'anthropic') {
    return `instale/autentique o CLI e valide com ${command} --version`;
  }
  if (providerMode === 'copilot') {
    return `instale/autentique o CLI e valide com ${command} --version`;
  }
  return 'provider nao reconhecido';
}
