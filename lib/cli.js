...
const { detectCircularImports } = require('./circular-imports');
...
const { renderIssueExplanation, explainIssueKind, explainableKinds } = require('./issue-explainer');
const { startLspServer } = require('./lsp-server');

function resolvePinguVersion() {
  try {
...
const { resolveProjectContext } = require('./project-context');
...
function runLegacyRuntime(rawRuntimeArgs) {
  ...
  if (!args.guardMode && !args.lspAiFixMode && !args.promptTaskMode && !args.analyze && !args.stdin && !args.serveMode && !args.lspMode) {
    process.exit(1);
  }

  if (args.lspMode) {
    startLspServer({ version: resolvePinguVersion() });
  } else if (args.serveMode) {
    startServer();
...
function resolveCliCommand(rawArgs) {
  ...
  if (['analyze', 'a', 'fix', 'f', 'prompts', 'prompt', 'comments', 'comment', 'p', 'taxonomy', 'offline', 'profile', 'init', 'context', 'ctx', 'doctor', 'explain', 'lsp', 'help'].includes(normalized)) {
...
function runCliCommand(command, commandArgs) {
  ...
  if (command === 'context' || command === 'ctx') {
    renderContext(resolveCliCommandInputs(commandArgs).args);
    return;
  }
  if (command === 'explain') {
    const commandInput = resolveCliCommandInputs(commandArgs);
    renderExplain(commandInput.positionals[0], commandInput.args);
    return;
  }
  if (command === 'lsp') {
    startLspServer({ version: resolvePinguVersion() });
...
function runAnalyzeTargets(targets, args) {
  ...
  const perFileIssues = targetFiles.flatMap((targetFile) => {
  ...
  // Importacao circular e uma analise multi-arquivo: so faz sentido com mais de
  // um arquivo no alvo (na analise por buffer do editor nao ha grafo de modulos).
  const crossFileIssues = targetFiles.length > 1
    ? detectCircularImports(targetFiles)
    : [];
  const issues = [...perFileIssues, ...crossFileIssues];
...
function renderExplain(kind, args) {
  const normalized = String(kind || '').trim();
  if (args.output === 'json') {
    const detail = explainIssueKind(normalized);
    renderJson(detail || { kind: normalized, found: false, available: explainableKinds() });
    return;
  }
  if (!normalized) {
    console.log([
      'Uso: pingu explain <kind>',
      '',
      'Kinds com explicacao disponivel:',
      ...explainableKinds().map((entry) => `  ${entry}`),
    ].join('\n'));
    return;
  }
  console.log(renderIssueExplanation(normalized));
}
...
function renderDoctor(args) {
  ...
  console.log(`- contexto: ${context.exists ? context.file : `${context.file} (ausente)`}`);
  if (context.suggestions.checkCommand) {
    console.log(`- check sugerido: ${context.suggestions.checkCommand}`);
  }
...
function renderContext(args) {
  const payload = resolveProjectContext({
    cwd: process.cwd(),
    write: args.write === true,
  ...
  console.log('Pingu context');
  console.log(`- root: ${payload.projectRoot}`);
  console.log(`- file: ${payload.file}`);
  console.log(`- status: ${payload.exists ? 'existe' : 'ausente'}`);
  if (payload.created) {
    console.log('- contexto criado');
  } else if (payload.overwritten) {
    console.log('- contexto sobrescrito');
  } else if (!payload.exists) {
    console.log('- use pingu context --write para criar');
  }
  if (payload.suggestions.checkCommand) {
    console.log(`- check sugerido: ${payload.suggestions.checkCommand}`);
  }
}
...
function buildDoctorChecks() {
  ...
  const providerCommand = String(process.env.PINGU_COPILOT_COMMAND || 'copilot').trim();
  const providerHint = providerAvailable
    ? 'operacional'
    : `instale/autentique o CLI e valide com ${providerCommand} --version`;
  ...
  return [
    ...
    },
    {
      ...
      message: providerAvailable
        ? `${providerName} operacional`
        : `${providerName} indisponivel: ${providerHint}`,
      details: {
        active: providerName,
...
function renderHelp() {
  console.log([
    ...
    '  pingu taxonomy [--json]',
    '  pingu explain <kind> [--json]',
    '  pingu offline [--json]',
    '  pingu profile [--lines 180] [--json]',
    '  pingu init [--force] [--json]',
    '  pingu context [--write|--force] [--json]',
    '  pingu doctor [--json]',
    '  pingu lsp',
    '',
