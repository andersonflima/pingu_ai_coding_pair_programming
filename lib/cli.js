...
function runLegacyRuntime(rawRuntimeArgs) {
  ...
  if (!args.guardMode && !args.lspAiFixMode && !args.analyze && !args.stdin && !args.serveMode) {
    process.exit(1);
  }
...
function buildDoctorChecks() {
  ...
  return [
    ...
    },
    {
      ...
      ok: ['auto', 'openai', 'copilot'].includes(providerMode),
...
function renderHelp() {
  console.log([
    ...
    '  realtime_dev_agent.js --lsp-ai-fix',
  ].join('\n'));
}
