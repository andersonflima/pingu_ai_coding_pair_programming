...
// f = open(...) sem `with`: o arquivo nao e fechado de forma garantida (vaza em
// caso de excecao). Conservador: so a forma de atribuicao direta, fora de `with`.
function checkPythonUnclosedResource(line, file, kind, lineNumber) {
  if (!isPythonLikeExtension(kind)) {
    return null;
  }
  const match = String(line || '').match(/^(\s*)([A-Za-z_]\w*)\s*=\s*open\s*\(/);
  if (!match || /\bwith\b/.test(line)) {
    return null;
  }
  return {
    file,
    ...
    severity: 'warning',
    kind: 'resource_leak',
    message: `Arquivo de '${match[2]}' aberto sem 'with'`,
    suggestion: 'Use o context manager `with open(...) as ...:` para garantir o fechamento do arquivo mesmo em caso de erro.',
    snippet: '',
    action: { op: 'insert_before' },
  };
}
