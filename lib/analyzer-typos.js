...
function applyTypoCorrections(code, corrections, found) {
  return corrections.reduce((acc, [typo, correction]) => {
    // Um unico regex global serve tanto para o teste quanto para o replace
    // (replace com flag global ignora o lastIndex de entrada).
    ...
    return acc.replace(pattern, correction);
  }, code);
}
