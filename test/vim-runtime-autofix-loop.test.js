...
test('runtime mantem hover automatico de issue passivo por padrao', () => {
  ...
  assert.match(internalRuntime, /s:script_call_rhs\('pingu_issue_hover_action\("preview"\)'\)/);
  assert.match(internalRuntime, /s:script_call_rhs\('pingu_issue_hover_action\("ai"\)'\)/);
  assert.match(internalRuntime, /s:script_call_rhs\('pingu_issue_hover_action\("explain"\)'\)/);
  ...
  assert.match(internalRuntime, /if l:focus_menu && \(!exists\('\*nvim_win_is_valid'\) \|\| nvim_win_is_valid\(l:winid\)\)\n    call s:start_pingu_issue_hover_ai_suggestion\(a:issue, l:signature\)/);
