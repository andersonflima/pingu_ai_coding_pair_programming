...
- no Neovim, diagnostics do LSP sem `codeAction` aplicavel podem usar `lsp_ai_fix` para gerar uma edicao local minima; para `reportUndefinedVariable`/simbolo indefinido, o payload assistido procura primeiro simbolos existentes no projeto para sugerir import e, quando nao houver candidato seguro, orienta a criacao da menor definicao local faltante
...
- quando o `apply` explicito de um diagnostico LSP executa uma code action que nao altera nenhum buffer carregado, o Pingu registra o no-op em `:PinguLogs` e tenta o fallback assistido da mesma linha antes de reportar falha, usando IA para escolher entre import, stub local ou edicao minima segura
